...
//实现一个回调，用于确认消息是否送达了QQ物联后台服务器
void on_send_test_notify_msg_ret(unsigned int cookie , int err_code )
{
...
/*
 * 发送强消息报警, 需要在物联网官网上配置触发器为3 并且显示强消息
 */
void test_send_notify_alarm()
{
	int msg_id = 3; 
	//char* msg  = "测试警告by6K";
	tx_send_notify_msg(msg_id,"测试警告by6K",on_send_test_notify_msg_ret,0,0,0);
}
