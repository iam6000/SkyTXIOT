...
extern void test_send_notify_alarm();
...
bool initDevice() {
	...
	int nLicenseSize = 0;
	if (!readBufferFromFile("./licence.sign.file[Hawkeye000000000].txt", license, sizeof(license), &nLicenseSize)) {
	...
	int nGUIDSize = 0;
	if(!readBufferFromFile("./Guid_file[Hawkeye000000000].txt", guid, sizeof(guid), &nGUIDSize)) {
    ...
    int nPubkeySize = 0;
    if (!readBufferFromFile("./1700002286.pem", svrPubkey, sizeof(svrPubkey), &nPubkeySize))
    {
...
int main(int argc, char* argv[]) {
	...
	while (scanf("%s", input)) {
		...
		{
			//todo  send notify alarm
			printf("<6K-DEBUG> begin send notify alarm test!!");
		 //	test_send_notify_alarm();
		}
		sleep(1);
	}
